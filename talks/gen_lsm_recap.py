...
# All boxes are vertically centered around y = 130.
# Unit = 80×80 (memtable and each L0 run).
# L1 = 2× unit (≈ 160×100). L2 ≈ 2.2× L1 (≈ 280×130).

# Memtable (1 unit)
MT_X, MT_Y, MT_W, MT_H = 30, 90, 80, 80
...
# Arrow: write
d.arrow("a_w", MT_X, 70, [[0, 0], [70, 0]], color=NAVY, sw=2, roughness=0)
d.text("a_w_t", MT_X, 46, 70, 18, "запись", size=13, color=NAVY)

# Arrow: flush
A1_X = MT_X + MT_W + 10
d.arrow("a_f", A1_X, 130, [[0, 0], [50, 0]], color=ORANGE, sw=2, roughness=0)
d.text("a_f_t", A1_X, 106, 50, 18, "дамп", size=13, color=ORANGE)

# L0 — three flushed files, each 1 unit
L0_X = A1_X + 60; L0_Y = 90; L0_W = 80; L0_H = 80; L0_GAP = 8
for i in range(3):
...
# Arrow: L0 → L1
A2_X = L0_END + 10
d.arrow("a_m", A2_X, 130, [[0, 0], [50, 0]], color=ORANGE, sw=2, roughness=0)
d.text("a_m_t", A2_X - 15, 106, 80, 18, "слияние", size=13, color=ORANGE)

# L1 ≈ 2 units
L1_W, L1_H = 160, 100
L1_X = A2_X + 60; L1_Y = 130 - L1_H // 2
...
# Arrow: L1 → L2
A3_X = L1_X + L1_W + 10
d.arrow("a_m2", A3_X, 130, [[0, 0], [50, 0]], color=ORANGE, sw=2, roughness=0)

# L2 ≈ 2.2× L1
L2_W, L2_H = 280, 130
L2_X = A3_X + 60; L2_Y = 130 - L2_H // 2
